angular.module('Workforce.controllers', [])
    ...
    .controller('welcomeCtrl', function ($scope, $state, LoginService, JobService) {
        ...
        if (angular.isDefined(window.localStorage['WorkForceUserMail'])) {
            var email = window.localStorage['WorkForceUserMail'];
            var password = window.localStorage['WorkForceUserPW'];
